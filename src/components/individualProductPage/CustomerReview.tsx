...
export default function CustomerReview({ Product }: Props) {
  ...
  const FiveStar =
    (Product?.review_count[0]?.five_star /
      Number(Product?.review_count[0].total_reviews)) *
    100;
  const FourStar = (Product?.review_count[0]?.four_star / Number(Product?.review_count[0].total_reviews)) * 100;
  const ThreeStar =
        ...
        return(
          <div className="container mx-auto px-4 lg:px-44 py-6">
            <div className=" space-y-6">
              ...
              <div>
                <h2 className="font-bold text-xl text-title text-center">No reviews yet</h2>
              </div>
            </div>
          </div>
        )
